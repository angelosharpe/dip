...
Toto CD obsahuje:
\begin{itemize}
\item Zdrojové kódy programu s~implementovaným SVM a Bayesovským klasifikátorem.\\ (Adresáø \texttt{/program/})
...
Detailnìj¹í popis jednotlivých operací s~Bayesovským klasifikátorem lze vypsat spu¹tìním pøíkazu: \texttt{./control.py bayes [operace] -h}, kde namísto slova \texttt{[operace]} napí¹eme název po¾adované operace.
...
Stejnì jako v~pøípadì Bayesovského klasifikátoru lze i pro SVM klasifikátor vypsat seznam operací, které lze s~SVM klasifikátorem provádìt.
...
\item Smutná ústa emotikonu -- \\
\verb`    sad_mouths = r'[/S\(\[\\\|cC@{]'`
\item Veselá ústa emotikonu -- \\
\verb`    happy_mouths = r'[dD\)\]pP}3]'`
\item Ostatní ústa emotikonu -- \\
...
\begin{itemize}
\item V¹echny ostatní emotikony -- \\
\verb`    other_emoticons = [`\\
\verb`        r'\^_+\^', r'o\.O', r'@_+@',`\\
\verb`        r'-_+-', r'\.\.+', r',,+', r'<3'`\\
\verb`    ]`
\item Ostatní smutné emotikony -- \\
\verb`    other_sad_emoticons = [r'-_+-']`
\item Ostatní veselé emotikony -- \\
\verb`    other_happy_emoticons = [r'\^_+\^', r'<3']`
\item Ostatní ostatní emotikony -- \\
