...
Obì metody jsou v~této práci podrobnì rozebrány, a~to jak z~teoretického, tak z~praktického hlediska.
...
Základ této práce byl rozpracován v~semestrálním projektu \cite{sep}, jen¾ byl vytvoøen v~rámci stejnojmenného pøedmìtu na FIT VUT.

Semestrální projekt obsahoval úvodní kapitolu o~problematice zpracování pøirozeného jazyka a popis problematiky Bayesovského klasifikátoru, jen¾ byl pro tuto práci také implementován (viz kapitola \ref{BAYES}).
Byla vytvoøena malá testovací sada pro trénování a testování tohoto klasifikátoru.
Na této datové sadì byly pro Bayesovský klasifikátor následnì provedeny testy, v~nich¾ byl zhodnocen vliv zvolených pøíznakù (viz kapitola \ref{TOKENIZACE}) na klasifikaèní schopnosti implementovaného Bayesovského klasifikátoru.
V~semestrálním projektu je¹tì nebyly implementovány speciální pøíznaky (viz kapitola \ref{TOKENIZACE-SPEC}).



\chapter{Úvod do klasifikace dokumentù}
V~této kapitole se budeme struènì zabývat historií (viz kapitola \ref{NLP-H}) oboru \emph{zpracování pøirozeného jazyka (natural language processing, dále té¾ NLP)}, poté hlavními úkoly, které se v~oboru zpracování pøirozeného jazyka øe¹í (viz kapitola \ref{NLP-HU}), dále popí¹eme typické klasifikaèní úlohy (viz kapitola \ref{NLP-KU}) a na závìr se budeme vìnovat problematice klasifikace dokumentù, je¾ je hlavním tématem této práce.
...
V~této práci se v¹ak budeme podrobnìji zabývat pouze metodou SVM, která bude dále podrobnìji popsána (viz kapitola \ref{SVM}).
...
Z~definice klasifikace (viz kapitola \ref{CLASS-DEF}) víme, ¾e klasifikaèní úloha je definována klasifikaèní funkcí $f$, která vstupnímu vzorku $x$ podle jeho pøíznakù pøiøadí oznaèení tøídy $y$, do ní¾ vzorek spadá.
...
\begin{itemize}
 ...
 \item \textit{Uèení bez uèitele (unsupervised learning)} -- pøi uèení bez uèitele je uèícímu se programu pøedána sada trénovacích dat, ve které si uèící se program sám hledá význaèné vlastnosti, na jejich¾ základì poté vytvoøí vlastní klasifikaèní funkci. Jedním z~mo¾ných pøístupù k~øe¹ení této metody je metoda shlukování (viz kapitola \ref{CLASS-APPROACHES-CLUSTERING}).
...
Zjednodu¹ení spoèívá v~tom, ¾e existence nebo naopak neexistence jednoho pøíznaku (textového nebo speciálního -- viz kapitola \ref{TOKENIZACE}) není závislá na existenci nebo neexistenci jiného pøíznaku.
...
V~posledních letech ale ji¾ existují jiné a lep¹í metody pro klasifikaci, napøíklad metoda \textit{support vector machines (SVM)}, která je v~této práci podrobnì popsána (viz kapitola \ref{SVM}).
...
\end{equation}
Nyní aplikujeme onu naivitu, kterou jsme zmiòovali vý¹e (viz kapitola \ref{BAYES}).
...
Pøi pou¾ití jaderné funkce vy¹¹ích øádù ale mù¾e nastat takzvané \emph{pøeuèení (overfitting)}, kdy se SVM klasifikátor nauèí na velmi specifické pøíznaky, které nejsou pro korektní klasifikaci smìrodatné, co¾ zpùsobí markantní zhor¹ení schopností SVM klasifikátoru.
...
Nevhodným výbìrem této promìnné mù¾e podobnì jako v~pøípadì polynomiální jaderné funkce dojít k~pøeuèení, proto je dùle¾ité zvolit její vhodnou hodnotu.
...
Skalární souèin $x^T_i x_j$ v~maximalizaèní rovnici mù¾eme nahradit libovolnou jadernou funkcí a dosáhnout tím nelineární transformace, a tedy i velké maximální ¹íøky ve vysoce dimenzionálních prostorech pøíznakù (viz kapitola \ref{SVM-THEORY-KERNELS}).
...
Díky této vlastnosti jsou SVM ménì náchylné k~pøeuèení a také klasifikaèní model, který tyto vektory tvoøí, je díky tomu velmi malý a rychlý.
...
Obecnì je tøeba optimálnì urèit parametr $C$ (míru chybovosti vzhledem k~maximalizaci ¹íøky (marginu) -- viz kapitola \ref{SVM_C}) a parametr pou¾ité jaderné funkce (urèující vlastnosti pou¾ité jaderné funkce -- viz kapitola \ref{SVM-KERNELS}), jen¾ budeme nadále nazývat $\gamma$.
...
Pro výpoèet energie tohoto po¾adavku $E_{SV}$ dostaneme následující vzorec:
...
Pro pøevedení slov do obecnìj¹ího tvaru lze kromì \emph{stematizace} pou¾ít také takzvané \emph{lematizace}, která na rozdíl od stematizace nepøevede slova na koøeny, ale do jejich základního tvaru. 
...
Nevýhodou tohoto pøístupu je, ¾e pro velké uèící mno¾iny výraznì roste velikost slovníku u~Bayesovského klasifikátoru a u~SVM klasifikátoru se zvìt¹uje poèet dimenzí pøíznakù, co¾ vede k~obtí¾nìj¹ímu nalezení optimální dìlící nadroviny a mù¾e vést i k~pøeuèení.
...
Pro rozdìlení vstupního textu na jednotlivé vìty vyu¾íváme knihovnu NLTK (viz. kapitola \ref{IMP-NLTK}).
...
\begin{enumerate}
 ...
 \item \emph{Pøevedení slov na jejich koøeny (stematizace)} -- druhým modulem z~této knihovny je modul \texttt{snowball} (\texttt{nltk.stem.snowball}), jeho¾ úkolem je z~jednotlivých slov vstupního textu udìlat koøen slov (více viz kapitola \ref{Textove_tokeny}).
\end{enumerate}
...
Mezi tyto dal¹í knihovny patøí i balík knihoven \texttt{CVXOPT} (viz kapitola \ref{IMP-CVXOPT}), pou¾itý pro implementaci SVM klasifikátoru v~praktické èásti této práce.
...
Jeliko¾ jak Bayesovský klasifikátor, tak klasifikátor zalo¾ený na support vector machines pracují se vstupním textem prakticky stejnì, byla vytvoøena tøída \texttt{Entry} a tøídy dìdící z~rodièovské tøídy \texttt{Feature} (viz obrázek \ref{features-diag}), nacházející se v~adresáøi \texttt{common}.
...
Tato tøída tedy zodpovídá za vyhledávání jednotlivých speciálních pøíznakù (viz kapitola \ref{TOKENIZACE-SPEC}), které potom jako instance tøíd, dìdících ze tøídy \texttt{Features} spoleènì s~$n$-ticemi jednotlivých textových pøíznakù (viz kapitola \ref{Textove_tokeny}), pøedává klasifikátorùm.
...
Podobnì jako Bayesovský klasifikátor, tak i klasifikátor zalo¾ený na metodì support vector machines, potøebuje tøídu, je¾ pracuje se vstupním textem a~s~pøíznaky z~nìj vytvoøenými a pøipraví pøíznaky do podoby, ve které je klasifikátor schopen s~nimi pracovat.
...
Více o~tomto výbìru vhodných parametrù bylo popsáno v~kapitole o~SVM klasifikátoru (viz kapitola \ref{annealing}).
...
V~této èásti kapitoly se budeme podrobnìji zabývat postupem implementace jednotlivých klasifikaèních algoritmù.
...
Jak ji¾ bylo zmínìno vý¹e v~této kapitole o~pou¾itých knihovnách (viz kapitola \ref{IMPL_LIB}), pro øe¹ení optimalizaèního problému, vycházejícího z~matematického popisu klasifikátoru SVM (viz kapitola \ref{SVM}), byl pou¾it balík knihoven pro programovací jazyk Python s~názvem \texttt{CVXOPT}, pøesnìji modul \texttt{qp} z~tohoto balíku knihoven, umo¾òující øe¹it optimalizaèní problémy pomocí kvadratického programování.
...
Jestli¾e v¹ak pou¾íváme jadernou funkci $K(x_1, x_2)$, poèítající skalární souèin v~prostoru pøíznakù (viz kapitola \ref{SVM-KERNELS}), potom musíme nahradit skalární souèin $x^T_mx_n$ za tuto jadernou funkci, do ní¾ dosadíme jednotlivé vektory.
...
V~programu je mo¾né zvolit, zda chce u¾ivatel pou¾ívat promìnnou $C$ (viz kapitola \ref{SVM-THEORY-MARGINS}), tedy zda umo¾ní poru¹ovat maximální ¹íøku (margin).
...
Parametr $G$ a $h$ se tedy oproti pøedchozímu pøípadu bude muset roz¹íøit tak, aby pojal i pravou èást podmínky.
...
S~tìmito Lagrangeovými multiplikátory pak dále pracujeme a vytvoøíme z~nich klasifikaèní model (viz kapitola \ref{SVM-THEORY-MARGINS}).
...
V~takovém pøípadì je pro výpoèet Gramovy matice $G$ pou¾ita, namísto standardního kartézského souèinu v~prostoru pøíznakù $X$, jaderná funkce, která ale vlastnì pøedstavuje kartézský souèin v~prostoru $F$, do kterého lze prostor $X$ pøevést pomocí urèité transformaèní funkce $\varphi$ (viz kapitola \ref{SVM}).
...
poté se zamìøíme na testování rùzných typù pøíznakù (viz kapitola \ref{TOKENIZACE}).

Závìrem této kapitoly porovnáme oba implementované klasifikátory a shrneme výsledky, ke kterým jsme pøi vyhodnocování testù dospìli.

\section{Data}\label{TEST-DATA}
Jak ji¾ bylo zmínìno v~úvodu (viz kapitola \ref{UVOD-MOTIVACE}), tato práce vznikla s~cílem vytvoøit klasifikátor textu pro projekt M-Eco~\footnote{\url{http://www.meco-project.eu/}}, ve kterém participovala výzkumná skupina NLP, pracující pøi Fakultì informaèních technologii VUT v~Brnì.
...
Úkolem klasifikátoru v~projektu M-Eco byla klasifikace vstupního textu do tøíd a následné urèení, zda bude vstupní text zapsán do databáze, nebo nebude.
...
Jedna datová sada byla vytvoøena pro anglické tweety (textové pøíspìvky ze sociální sítì Twitter) a druhá pro anglické èlánky (z~internetových periodik).
...
Ke zvolení tìchto tøíd u~obou datových sad vedl pøedpoklad, ¾e i z~nespecializovaných veøejnì pøístupných zdrojù lze získat informace o~výskytu nemocí ve svìtì a postupu a roz¹íøení epidemií apod.

Jak vychází z~odvození klasifikaèní rovnice, pou¾ité v~Bayesovském klasifikátoru (viz vzorec \ref{SF-BAYES-MAT-bayes_theorem-deriv}), mìlo by rozlo¾ení relevantních a nerelevantních dat v~trénovací a testovací mno¾inì být zhruba v~pomìru 1:1.
...
\begin{itemize}
 \item \textit{Pravdivì pozitivní (true positive)} -- poèet vstupních textù, které byly klasifikátorem správnì zaøazeny do relevantní tøídy.
 \item \textit{Pravdivì negativní (true negative)} -- poèet vstupních textù, které byly klasifikátorem správnì zaøazeny do nerelevantní tøídy.
...
Pro v¹echny vý¹e zmínìné metody porovnání klasifikátorù se pøedpokládá, ¾e jsme schopni vypoèítat hodnoty \textit{pravdivì pozitivní}, \textit{pravdivì negativní}, \textit{fale¹nì pozitivní} a \textit{fale¹nì negativní} (viz kapitola \ref{TEST-VYS_KLAS}).
...
$$
  CP = \frac{pravdive\_pozit. + pravdive\_negat.}{pravdive\_pozit. + pravdive\_negat. + falesne\_pozit. + falesne\_negat.} .
$$
...
$$
  Presnost = \frac{pravdive\_pozitivni}{pravdive\_pozitivni + falesne\_pozitivni}
$$
$$
  Pokryti = \frac{pravdive\_pozitivni}{pravdive\_pozitivni + falesne\_negativni}.
$$
...
Pro testování klasifikace anglických tweetù byla pou¾ita datová sada manuálnì anotovaných tweetù a pro testování klasifikace anglických èlánkù byla pou¾ita datová sada manuálnì anotovaných èlánkù (viz kapitola \ref{TEST-DATA}).

\subsection{Testy textových pøíznakù}
Pro zji¹tìní ideální délky $n$-tic textových pøíznakù jsme aplikovali testy pro postupnì se zvy¹ující $n$, a to, dokud se zlep¹ovala hodnota Pearsonova korelaèního koeficientu a F1-measure (viz kapitola \ref{TEST-METRIKY}).
...
Následující tabulka \ref{TEST_TAB1} prezentuje, jak se klasifikátor choval pøi zmìnì nastavení maximální délky za sebou jdoucích $n$-tic textových pøíznakù:
...
n                       & 1      & 2      & 3      & 4 \\
\midrule
Pravdivì pozitivní       & 83,2 (41,6\%) & 90,0 (45,0\%)  & 90,0 (45,0\%)  & 90,0 (45,0\%)  \\
Pravdivì negativní       & 93,8 (46,9\%) & 91,2 (45,6\%)  & 91,2 (45,6\%)  & 91,2 (45,6\%)  \\
Fale¹nì pozitivní     &  6,2  (3,1\%) &  8,8 (4,4\%)  &  8,8 (4,4\%)  &  8,8 (4,4\%)  \\
Fale¹nì negativní     & 16,8  (8,4\%) & 10,0 (5,0\%)  & 10,0 (5,0\%)  & 10,0 (5,0\%)  \\
...
\end{tabular}
\end{center}
\caption{Vliv rùzných délek $n$-tic textových pøíznakù na Bayesovský klasifikátor pøi klasifikaci tweetù.}
\label{TEST_TAB1}
\end{table}
...
Sna¾ili jsme se urèit optimální délku $n$-tic textových pøíznakù.
Výsledky jednotlivých testù jsou zaznamenány v~následující tabulce \ref{TEST_TAB2}:
...
n                       & 1      & 2      & 3      & 4 \\
\midrule
Pravdivì pozitivní       & 21,0 (28,4\%)  & 21,4 (28,9\%)  & 21,4 (28,9\%)  & 21,4 (28,9\%)  \\
Pravdivì negativní       & 25,6 (34,6\%)  & 29,4 (39,7\%)  & 29,4 (39,7\%)  & 29,4 (39,7\%)  \\
Fale¹nì pozitivní     & 11,4 (15,4\%)  & 7,6  (10,3\%)  & 7,6  (10,3\%)  & 7,6  (10,3\%)  \\
Fale¹nì negativní     & 16,0 (21,6\%)  & 15,6 (21,1\%)  & 15,6 (21,1\%)  & 15,6 (21,1\%)  \\
...
\end{tabular}
\end{center}
\caption{Vliv rùzných délek $n$-tic textových pøíznakù na Bayesovský klasifikátor pøi klasifikaci èlánkù.}
\label{TEST_TAB2}
\end{table}
...
V~implementovaném programu byla vytvoøena metoda, která vliv v¹ech mo¾ných speciálních pøíznakù na klasifikaci porovná a najde jejich optimální kombinaci.
...
Pro datovou sadu anglických tweetù byl otestován vliv jednotlivých speciálních pøíznakù (viz kapitola \ref{TOKENIZACE-SPEC}) na výsledek klasifikace Bayesovského klasifikátoru.
Následující tabulka \ref{TEST_TAB3} popisuje, jakou mìrou jednotlivé pøíznaky zlep¹ovaly nebo zhor¹ovaly výsledek klasifikace Bayesovského klasifikátoru oproti kontrolnímu testu.
...
\begin{tabular}{lc}
\toprule
spec pøíznaky (viz kapitola \ref{TOKENIZACE-SPEC}) & Zmìna korelace \\
\midrule
URL -- celé                      & -0,0014 \\
URL -- doména                    & -0,0203 \\
...
\end{tabular}
\end{center}
\caption{Vliv jednotlivých speciálních pøíznakù na zmìnu korelace výsledkù Bayesovského klasifikátoru pøi klasifikaci tweetù.}
\label{TEST_TAB3}
\end{table}


Z~tabulky \ref{TEST_TAB3} vyplývá, ¾e optimální speciální pøíznaky pro datovou sadu anglických tweetù jsou následující:
...
Následující tabulka \ref{TEST_TAB4} popisuje, jak se zlep¹í klasifikaèní schopnosti Bayesovského klasifikátoru  pøi pou¾ití vý¹e zmínìných optimálních speciálních pøíznakù pøi trénování a testování na datové sadì tweetù.
...
\midrule
Pravdivì pozitivní       & 90,2 (45,1\%)              & 90,2 (45,1\%)  \\
Pravdivì negativní       & 90,6 (45,3\%)              & 91,2 (45,3\%)  \\
Fale¹nì pozitivní     &  9,4  (4,7\%)              &  8,8  (4,4\%)  \\
Fale¹nì negativní     &  9,8  (4,9\%)              &  9,8  (4,9\%)  \\
...
\end{tabular}
\end{center}
\caption{Porovnání výsledkù klasifikace tweetù pomocí Bayesovského klasifikátoru bez pou¾ití speciálních pøíznakù a s pou¾itím speciálních pøíznakù.}
\label{TEST_TAB4}
\end{table}

Z~tabulky \ref{TEST_TAB4} vyplývá, ¾e optimálnì zvolené speciální pøíznaky mohou mírnì vylep¹it klasifikaèní schopnosti Bayesovského klasifikátoru pøi klasifikaci anglických tweetù.


\subsubsection{Èlánky}
% no-features:{'emoticon': 4, 'sentence': 1, 'url': 4, 'tag': 3, 'time': 3, 'date': 4, 'user_tag': 3, 'email': 3}
% best-fatures: {'emoticon': 4, 'sentence': 1, 'url': 4, 'tag': 3, 'time': 3, 'date': 1, 'user_tag': 0, 'email': 0}
Obdobnì jako pøi klasifikaci tweetù i díky vhodnému výbìru speciálních pøíznakù docházelo ke zlep¹ení klasifikaèních schopností klasifikátoru, co¾ popisuje následující tabulka.
V~tomto pøípadì ale testy klasifikátoru probìhly nad datovou sadou anglických èlánkù.
V~následující tabulce \ref{TEST_TAB5} je prezentováno do jaké míry jednotlivé speciální pøíznaky zlep¹ovaly, nebo zhor¹ovaly výsledek klasifikace oproti kontrolnímu výsledku.
...
Z~tabulky \ref{TEST_TAB5} vyplývá, ¾e byly zvoleny následující optimální speciální pøíznaky, které nejlépe napomáhají zlep¹ení klasifikaèních schopností Bayesovského klasifikátoru pøi klasifikaci datové sady anglických èlánkù:
...
Ostatní skupiny speciálních pøíznakù, které zde nebyly zmínìny (URL, emotikony, tagy, vìty a èasy), nezlep¹ily výsledky klasifikace Bayesovského klasifikátoru nad datovou sadou anglických èlánkù, a proto nebyly pou¾ity.
...
\toprule
                        & Bez spec. pøíznakù & S~optim. spec. pøíznaky  \\
\midrule
Pravdivì pozitivní       & 21,4 (28,9\%)              & 21,4 (28,9\%)  \\
Pravdivì negativní       & 29,4 (39,7\%)              & 29,4 (39,7\%)  \\
Fale¹nì pozitivní     &  7,6 (10,3\%)              &  7,6 (10,3\%)  \\
Fale¹nì negativní     & 15,6 (21,1\%)              & 15,6 (21,1\%)  \\
...
\end{tabular}
\end{center}
\caption{Porovnání výsledkù klasifikace èlánkù pomocí Bayesovského klasifikátoru bez pou¾ití speciálních pøíznakù a s pou¾itím speciálních pøíznakù.}
\label{TEST_TAB6}
\end{table}

Z~tabulky \ref{TEST_TAB6} je zøejmé, ¾e pro datovou sadu èlánkù speciální pøíznaky sice o~nìco zlep¹ily korelaci klasifikátoru, nicménì na výsledcích klasifikace Bayesovského klasifikátoru se to nikterak neprojevilo.
...
Je mo¾né, ¾e pøi zvìt¹ení datové sady pro uèení by vliv speciálních pøíznakù na zlep¹ení klasifikaèních schopností Bayesovského klasifikátoru mohl narùst.
...
Na rozdíl od Bayesovského klasifikátoru ale nemusíme volit jaké speciální pøíznaky budou pro klasifikaci vyu¾ity (viz kapitola \ref{svm-features}).
...
\subsection{Tweety}
Následující tabulka \ref{TEST_TAB7} popisuje klasifikaèní schopnosti SVM klasifikátoru pro jednotlivé maximální délky $n$-tic textových pøíznakù.
...
n                       & 1      & 2      & 3      \\
\midrule
Pravdivì pozitivní       & 93,0 (46,5\%)   & 94,0 (47,0\%)  & 93,0 (46,5\%)  \\
Pravdivì negativní       & 92,8 (46,4\%)   & 91,4 (45,7\%)  & 91,8 (45,9\%)  \\
Fale¹nì pozitivní     &  7,2  (3,6\%)   & 8,6   (4,3\%)  & 8,2   (4,1\%)  \\
Fale¹nì negativní     &  7,0  (3,5\%)   & 6,0   (3,0\%)  & 7,0   (3,5\%)  \\
...
\end{tabular}
\end{center}
\caption{Vliv rùzných délek $n$-tic textových pøíznakù na SVM klasifikátor pøi klasifikaci tweetù.}
\label{TEST_TAB7}
\end{table}

Z~vý¹e uvedené tabulky \ref{TEST_TAB7} jasnì vyplývá, ¾e klasifikaèní schopnosti SVM klasifikátoru se se zvy¹ováním maximální délky $n$-tic textových pøíznakù zhor¹ovaly, tudí¾ není vhodné tyto $n$-tice pro klasifikaci tweetù pomocí SVM klasifikátoru pou¾ívat. 
...
To má potom za následek pøeuèení (=overfitting, viz kapitola \ref{SVM-KERNELS}).
...
Následující tabulka \ref{TEST_TAB8} popisuje výsledky klasifikace SVM klasifikátoru pro jednotlivé testované délky $n$-tic textových pøíznakù.
...
n                       & 1              & 2      \\
\midrule
Pravdivì pozitivní       & 24,4 (33,0\%)  & 19,8 (26,8\%)   \\
Pravdivì negativní       & 32,0 (43,2\%)  & 34,2 (46,2\%)  \\
Fale¹nì pozitivní     &  5,0  (6,8\%)  &  2,8 (3,8\%)  \\
Fale¹nì negativní     & 12,6 (17,0\%)  & 17,2 (23,2\%)  \\
Pøesnost                & 0,8299         & 0,8761 \\
Pokrytí                 & 0,6594         & 0,5351 \\
Celková pøesnost        & 0,7621         & 0,7297 \\
...
Z~tabulky \ref{TEST_TAB8} je tedy stejnì jako pøi klasifikaci tweetù jasné, ¾e nejlep¹ích klasifikaèních výsledkù dosahuje SVM klasifikátor kdy¾ jsou pou¾ívány pouze základní jednoslovné textové pøíznaky.
...
Následující tabulka \ref{TEST_TAB9} popisuje nejlep¹í výsledky Bayesovského klasifikátoru a SVM klasifikátoru na datové sadì tweetù:
...
\midrule
Pravdivì pozitivní       & 90,2 (45,1\%)            & 93,0 (46,5\%)      \\
Pravdivì negativní       & 91,2 (45,3\%)            & 92,8 (46,4\%)      \\
Fale¹nì pozitivní     &  8,8  (4,4\%)            &  7,2  (3,6\%)      \\
Fale¹nì negativní     &  9,8  (4,9\%)            &  7,0  (3,5\%)      \\
...
Jak z~tabulky \ref{TEST_TAB9} jasnì vyplývá, klasifikaèní schopnosti SVM klasifikátoru na datové sadì anglických tweetù jasnì pøedèí Bayesovský klasifikátor nauèený na stejné datové sadì, vyu¾ívající optimální kombinace speciálních pøíznakù.


Nyní opìt porovnáme Bayesovský klasifikátor s~SVM klasifikátorem, tentokrát v¹ak ale na datové sadì anglických èlánkù.
Následující tabulka \ref{TEST_TAB10} popisuje toto porovnání klasifikátorù:
...
Klasifikátor            & Bayesovský klasifikátor  & SVM klasifikátor   \\
\midrule
Pravdivì pozitivní       & 21,4 (28,9\%)            & 24,4 (33,0\%)      \\
Pravdivì negativní       & 29,4 (39,7\%)            & 32,0 (43,2\%)      \\
Fale¹nì pozitivní     &  7,6 (10,3\%)            &  5,0  (6,8\%)      \\
Fale¹nì negativní     & 15,6 (21,1\%)            & 12,6 (17,0\%)      \\
Pøesnost                & 0,7379                   & 0,8299             \\
Pokrytí                 & 0,5784                   & 0,6594             \\
Celková pøesnost        & 0,6865                   & 0,7621             \\
...
V~pøípadì anglických èlánkù je ale rozdíl mezi výsledky klasifikace Bayesovského klasifikátoru a SVM klasifikátoru velký -- Bayesovský klasifikátor dosáhl ohodnocení pomocí metriky F1-measure rovné 0,6485 a SVM klasifikátor dosáhl ohodnocení 0,7349, co¾ je pomìrnì velký rozdíl ve prospìch SVM klasifikátoru.
...
Z~provedených porovnání SVM a Bayesovského klasifikátoru je jasnì patrné, ¾e SVM klasifikátor je lep¹í metodou pro klasifikaci, která pøi testování na dvou velmi odli¹ných testovacích sadách dává v porovnání s Bayesovským klasifikátorem lep¹í výsledky v~pøípadì klasifikace anglických tweetù a výraznì lep¹í výsledky v~pøípadì klasifikace anglických èlánkù.


\chapter{Závìr} 
V~této diplomové práci jsme se zamìøili na dvì klasifikaèní metody pro klasifikaci textu -- SVM klasifikátor a Bayesovský klasifikátor.
...
Hlavním cílem projektu M-Eco bylo pomocí analýzy dat ze sociálních sítí, zpravodajských serverù, blogù a dal¹ích zdrojù na internetu hledat mo¾ná obecná zdravotní ohro¾ení (epidemie, pandemie, atd.).
Úkolem klasifikátoru v~projektu M-Eco byla klasifikace vstupního textu do tøíd a následné urèení, zda bude vstupní text zapsán do databáze, nebo nebude.
SVM klasifikátor implementovaný v~této diplomové práci v¹ak ji¾ bohu¾el pøi práci na tomto projektu pou¾it nemohl být, a to z~dùvodu ukonèení spolupráce na projektu M-Eco v~srpnu roku 2012.

Abychom co nejvìt¹í mìrou pozitivnì ovlivnili klasifikaèní schopnosti obou implementovaných klasifikátorù, bylo dbáno na výbìr vhodných pøíznakù, extrahovaných z~klasifikovaného textu.
...
Do speciálních pøíznakù potom patøily ostatní netextové pøíznaky, které byly z~textu získány -- napø. emotikony, poèet slov ve vìtì, e-mailové adresy atd. (viz kapitola \ref{TOKENIZACE}).
...
Pro SVM klasifikátor bylo tøeba mimo výbìru optimálních pøíznakù urèit optimální nastavení volných parametrù klasifikátoru a pou¾ité jaderné funkce (viz kapitola \ref{SVM}, \ref{SVM-KERNELS}).
...
Z~výsledkù tohoto porovnání plyne, ¾e SVM klasifikátor dosahuje pøi klasifikaci obou datových sad lep¹í pøesnosti klasifikace ne¾ Bayesovský klasifikátor (viz kapitola \ref{porovnani}).
...
S~velmi dobrými výsledky by zøejmì také mohla být pro oba implementované klasifikátory pou¾ita metoda zvaná boosting, popsaná v~èlánku Michaela Kearnse s~názvem \uv{Thoughts on hypothesis boosting} \cite{Kearns1988} (viz také kapitola \ref{boosting}).
Dal¹ím námìtem pro pøípadné roz¹íøení této práce by mohlo být vyu¾ití nové implementace SVM klasifikátoru nazavané v-SVM popsané v~èlánku \uv{New Support Vector Algorithms} \cite{Schölkopf98newsupport}, které by mohlo pøinést vylep¹ení klasifikaèních schopností SVM klasifikátoru.
