...
Úkolem automatické sumarizace je redukovat vstupní text nebo sadu textù do nìkolika slov, nebo krátkého odstavce, popisujícího sémantický obsah vstupního textu.
...
Po zji¹tìní významu slova je slovo nahrazeno synonymem.
...
V tomto èlánku se budeme zabývat právì pøedev¹ím metodami filtrování spamu, a to nejen pro pou¾ití pøi jeho odfiltrovávání z e-mailové po¹ty, ale také pro oznaèování relevantních textových vstupù vzhledem k danému tématu.
...
V \textit{multi-label} klasifikaci se ke ka¾dé tøídì mù¾e pøiøadit reálné èíslo $p \in <0,1>$, které definuje míru nále¾itosti klasifikovaného objektu do pøiøazených tøíd.
...
\subparagraph{Pravdìpodobnost pøíjmutí nového stavu}
S postupným sni¾ování teploty se z principu simulovaného ¾íhání sni¾ovat pravdìpodobnost skoku na jiné, ne¾ lep¹í stavy.
...
Pro urèení, zda se má do nového stavu skoèit, èi ne je vygenerováno náhodné èíslo $X \in <0,1>$ a pokud je $P_{skoku} > X$, pak je pro dal¹í iteraci pou¾it sousední stav jako stav základní.
...
Jedním z úkolù vyplývajících ze zadání této práce bylo vytvoøit datovou sadu pro trénování a testování obou zvolených klasifikátorù.
Zdrojem tìchto dat byla databáze tweetù projektu M-eco\footnote{http://www.meco-project.eu/}, ve kterém participovala výzkumná skupina NLP pracující pøi Fakultì informaèních technologii VUT v Brnì.

Byly vytvoøeny dvì datové sady, jedna pro anglické tweety a druhá pro anglické èlánky.
Jak vychází z odvození klasifikaèní rovnice pou¾ité v Bayesovském klasifikátoru (viz \ref{SF-BAYES-MAT-bayes_theorem-deriv}), mìlo by rozlo¾ení relevantních a nerelevantních dat v trénovací a testovací mno¾inì být zhruba 50/50.
Celkovì bylo anotováno asi 4500 anglických tweetù a asi 500 anglických èlánkù.
V databázi anotovaných anglických tweetù se nachází 502 relevantních a zbytek nerelevantních záznamù.
Pro databázi ruènì anotovaných anglických èlánku je tento pomìr 194 relevantních ku 269 nerelevantních záznamù.
Vzhledem k tomu, ¾e v testu chceme porovnávat dva klasifikátory, z nich¾ jeden má vlastnost limitující jeho uèící sadu, budeme pro oba klasifikátory pou¾ívat trénovací sadu o stejné velikosti -- tzn. pro tweety bude datová sada pro testy obsahovat 502 relevantních a 502 nerelevantních záznamù a pro èlánky bude obsahovat 194 relevantních a 194 nerelevantních záznamù.
...
Pøi testování v implementaèní èásti této práce byla pou¾ita metoda \emph{5-fold cross-validation}, tedy data byla rozdìlena na 5 stejných dílù a nad tìmito díly se provádìly testy.
Èíslo 5 bylo zvoleno kvùli poèítaèi, na kterém testy probíhaly.
Validaèní proces je paralelizován -- ka¾dá iterace v implementovaném programu vyu¾ívá právì jedno jádro a výsledky v¹ech jader jsou po dobìhnutí v¹ech iterací zprùmìrovány do jednoho výsledku.

\section{Úloha implementovaných klasifikátorù}
Jak bylo ji¾ zmínìno v úvodu práce (\ref{UVOD-MOTIVACE}), hlavním z dùvodu pro vytvoøení této práce byla potøeba vytvoøit klasifikátor pro projekt M-Eco, který klasifikuje vstupní textu do tøíd a urèuje tak, který text bude zapsán do  databáze a který nebude.

Pro datovou sadu obsahující anglické tweety jsou tedy implementované klasifikátory nauèeny k rozdìlování vstupních tweetù do dvou tøíd -- \textbf{zabývající se osobními zku¹enostmi pisatelù s nemocemi (ale také zku¹enostmi z pisatelova okolí)}, nebo \textbf{nezabývající se tìmito zku¹enostmi}.
...
Pro datovou sadu obsahující anglické èlánky jsou implementované klasifikátory  podobnì jako v pøípadì tweetù nauèeny k rozdìlování vstupních tweetù do dvou tøíd -- \textbf{zabývající se skuteènou nákazou ve svìtì}, nebo \textbf{nezabývající se skuteènou nákazou ve svìtì}.

\section{Testovací metriky} \label{TEST-METRIKY}
Aby bylo mo¾né nìjakým zpùsobem objektivnì porovnat rùzné výsledky klasifikátoru a také posléze klasifikátory mezi sebou, je tøeba zavést vhodné metriky popisující výsledky klasifikátoru.
...
Korelaci je tedy mo¾né poèítat pouze u Bayesovského klasifikátoru, který pøiøazuje ka¾dému klasifikovanému vstupu urèitou pravdìpodobnost.
U SVM klasifikátoru hodnota, která by pøiøazovala pravdìpodobnost nále¾itosti do urèité tøídy neexistuje, proto není mo¾né spoèítat korelaci.
...
Tyto hodnoty jsou výsledkem porovnání pøedpokládaného výstupu klasifikátoru s reálným výstupem implementovaného klasifikátoru.
...
V této kapitole budeme hledat optimální nastavení klasifikátoru pro klasifikaci jednotlivých typù textu (anglické tweety a èlánky).
...
Stejný test jako v pøípadì testu textové tokenizace tweetù.
Budeme se sna¾it urèit optimální délku délky $N$-tic textových tokenù 
...
\subsection{Testy pøíznakù}
Abychom mohli otestovat vliv jednotlivých typù pøíznakù na klasifikaci Bayesovského klasifikátoru, spustíme nejprve test klasifikátoru pouze vyu¾ívajíce textových tokenù.
Takto získáme kontrolní výsledek se kterým potom budeme moci porovnávat výsledky klasifikace s jednotlivými speciálními pøíznaky.
Pro porovnávání byla v implementovaném programu vytvoøena metoda, která vliv v¹ech mo¾ných speciálních pøíznakù porovná a najde nejoptimálnìj¹í kombinaci.

Zde pøedstavíme jakým zpùsobem je mo¾né zlep¹it klasifikaci výbìrem vhodných pøíznakù.
Pro ka¾dou datovou sadu bude ukázán výsledek bez speciálních pøíznakù a poté výsledek s vhodnì vybranými speciálními pøíznaky.

\subsection{Tweety}
Následující tabulka popisuje, jakým zpùsobem se zlep¹í klasifikaèní schopnosti klasifikátoru s vyu¾itím výbìru vhodných speciálních pøíznakù pøi trénování a testování na datové sadì tweetù.
...
Podíváme-li se na výsledky testù zabývajících se délkou textových tokenù, zjistíme, jak v pøípadì klasifikace èlánkù, tak v pøípadì klasifikace tweetù tokenizace tweetù je nejlep¹ím øe¹ení pou¾ít pro tokenizaci textu alternativní metodu tokenizace slov s maximální délkou $N$-tic rovnou 2.
...
Je-li délka $N$-tic vìt¹í ne¾ 2 klasifikátoru to u¾ v klasifikaci nepomáhá a pouze toto nastavení zvy¹uje velikost slovníku klasifikátoru, podle kterého potom klasifikátoru klasifikuje vstupní texty.
Dùvod pro to, ¾e mo¾nost del¹ích $N$-tic ji¾ nezlep¹uje klasifikaci je zøejmì ten, ¾e tøi za sebou jdoucí slova jsou ji¾ dosti specifickým pøíznakem, který se v textu nevyskytuje pøíli¹ èasto, a tudí¾ nejsou a¾ tak nápomocná.
Kdyby se výraznì zvìt¹ila trénovací a testovací datová sada, je mo¾nì, ¾e by i del¹í $N$-tice zlep¹ovaly klasifikaèní schopnosti klasifikátoru.

Testy zabývající se tokeny speciálních pøíznakù získávaných z textu ukázaly, ¾e v pøípadì tweetù mírnì zlep¹ují klasifikaèní schopnosti klasifikátoru.
V pøípadì klasifikace èlánkù ov¹em tak dobrý úspìch nemìly, sice klasifikaèní schopnost klasifikátoru zlep¹ily, ale tak málo, ¾e se to prakticky neprojevilo.
