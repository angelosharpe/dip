...
Toto téma diplomové práce jsem si zvolil, jeliko¾  jsem témìø dva roky pracoval na projektu M-eco\footnote{http://www.meco-project.eu/} ve skupinì NLP na VUT FIT, kde jsem se zabýval klasifikací tweetù\footnote{Tweety -- pøíspìvky zveøejòované na sociální síti Twitter (http://twitter.com/)}.
...
Bayesovský klasifikátor implementovaný této práci byl nasazen do reálného provozu v projektu M-Eco.
...
Mezi tyto knihovny patøi i \texttt{CVXOPT} (viz \ref{IMP-CVXOPT}) pou¾itý pro implementaci SVM klasifikátoru v praktické èásti této práce.

\subsection{CVXOPT} \label{IMP-CVXOPT}
\texttt{CVXOPT} je volnì dostupný balík knihoven pro øe¹ení konvexních optimalizaèních problému (mimo jiné do této skupiny problémù patøí i kvadratické programování jen¾ je pou¾ito v implementaci SVM klasifikátoru) v programovacím jazyce Python.
...
\subsection{Gram matice} \label{IMP-GRAM}
