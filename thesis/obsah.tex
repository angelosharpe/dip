...
Reálné vyu¾ití Bayesovského klasifikiátoru pøi této práci mne motivovalo k tomu, abych se klasifikaèními metodami zabýval dále.
...
Základ této práce byl rozpracován v semestrálním projektu \cite{sep}, jen¾ byl vytvoøen pro stejnojmenný pøedmìt.
...
Na této datové sadì a klasifikátorem byly následnì provedeny testy, v nich¾ byl zhodnocen vliv zvolených pøíznakù (viz \ref{TOKENIZACE}) na klasifikaèní schopnosti implementovaného Bayesovského klasifikátoru.
V odevzdané práci je¹tì nebyly implementovány speciální pøíznaky (viz \ref{TOKENIZACE-SPEC}).



\chapter{Úvod do klasifikace textù}
...
V této èásti kapitoly se podrobnìji podíváme na následující dvì klasifikaèní úlohy:
\begin{itemize}
...
Tøída v kontextu zpracovávání pøirozeného jazyka je mno¾ina objektù, vyznaèujících se urèitou spoleènou vlastností nebo vlastnostmi, která/ré danou tøídu popisuje/jí.
...
\section{Klasifikace a shlukování}\label{CLASS-APPROACHES-CLUSTERING}
...
Jeliko¾ pro vytvoøení funkèního klasifikátoru je nutné, aby klasifikátor byl nauèen pomocí správné trénovací mno¾iny, je strojové uèení s tématem klasifikace velmi úzce spojeno.
...
\subsection{Boosting}
V roce 1988 vyslovil Michael Kearns s názvem \uv{Thoughts on hypothesis boosting} \cite{Kearns1988} otázku, zda lze z mno¾iny slabých klasifikátorù (takových, které mají nízké hodnoty korelace testovacích a výstupních dat) vytvoøit jeden klasifikátor, který by mìl výraznì lep¹í výsledky.
...
Tudí¾, i kdy¾ existence nìkolika pøíznakù na sobì záviseí, Bayesovský klasifikátor s tìmito pøíznaky pracuje jako se zcela nezávislými událostmi.
...
V posledních letech ale ji¾ existují jiné a lep¹í metody pro klasifikaci, napøíklad metoda \textit{support vector machines}, která je v této práci podrobnì popsána (viz \ref{SVM}).
...
Pro klasifikaci textu není vhodné pou¾ít pøímo rovnici z Bayesova teorému, jeliko¾ by bylo tøeba zapamatovat si pro ka¾dý pøíznak tøi hodnoty a bylo by nutné provádìt velké mno¾ství výpoètù, nicménì je mo¾né rovnici upravit do tvaru, v nìm¾ je tøeba si pro ka¾dý pøíznaku pamatovat pouze jednu hodnotu pravdìpodobnosti a také není tøeba provádìt tolik výpoètù.
...
\end{equation}
kde $P$ je pravdìpodobnost urèující míru nále¾itostí klasifikované zprávy do tøídy spam a $p_i, i=1 \ldots N$ udává tuto míru pro jednotlivé pøíznaky $i$.
...
Mìjme $X$ a $Y$, pøedstavující dva pøíznaky, $S$ znamenající \uv{je to spam} a $\neg S$ znamenající \uv{je to ham} (=není to spam).
...
Bayesovský klasifikátor si do databáze pøi uèení ukládá pravdìpodobnosti zda jsou jednotlivé pøíznaky ze vstupních vìt spam, tzn. ukládá si hodnotu  $P(S|X)$.
...
Klasifikátor se podívá do databáze a zjistí pravdìpodobnost $P(S|X_i)$ se kterou jednotlivé pøíznaky nále¾í do tøídy spamu.
Jestli¾e klasifikátor pøi klasifikaci textu narazí na pøíznak, který dosud není v jeho databázi, pak je mu pøiøazena hodnota 0,5 (tzn. klasifikátor neumí urèit s jakou pravdìpodobností je tento pøíznak spam èi ham).
...
Tuto vìtu si klasifikátor rozdìlí podle zadaného pravidla na seznam pøíznakù (v tomto pøípadì pouzze slov).
$$['Aaaa', 'my', 'stomach', 'hurts']$$
V databázi klasifikátoru máme napøíklad:
$$\{'my':0.6, 'stomach':0.2, 'hurts':0.2\},$$
kde èísla za jednotlivými pøíznaky urèují pravdìpodobnost nále¾itosti odpovídajících pøíznakù do tøídy spamu.
...
Mimo NLP má v¹ak také velkou ¹kálu vyu¾ití, a» u¾ pro klasifikaèní úèely, nebo kupøíkladu pro úèely regresní analýzy (pro pøedvídání chování finanèních trhù, atd.).
...
Ka¾dý jeden záznam v trénovacích a testovacích datech má definovanou tøídu (\emph{label}), do které záznam spadá.
...
Ozanaèení tøídy $y$ ka¾dého záznamu z dat je definován jako $y \in \{1,-1\}$ v závislosti na tom, do které tøídy daný záznam spadá.
...
Pøi pou¾ití jaderné funkce vy¹¹ích øádù, ale mù¾e nastat takzvaný overfitting (pøeuèení), kdy se SVM klasifikátor nauèí na velmi specifické pøíznaky, které nejsou pro korektní klasifikaci smìrodatné, co¾ zpùsobí markantní zhor¹ení schopností SVM klasifikátoru.
...
Nyní definujme maximální ¹íøku (margin) volnìho prostoru mezi rozdìlovací hyperplochou $f$ a nejbli¾¹ími body v prostoru $X$ (viz obrázek \ref{SVM-THEORY-MARGINS}).
...
Abychom tedy dostali optimální rozdìlovací hranici s maximální ¹íøkou, budeme muset maximalizovat hodnotu maximální ¹íøky.
Tato operace je ov¹em ekvivalentní s minimalizací $\frac{1}{2}||w||^2$.
Nalezení specifické diskriminaèní funkce s maximální ¹íøkou je tedy ekvivalentní s následujícím optimalizaèním problémem:
...
Toto opatøení mù¾e nìkdy pomoci nalézt vìt¹í maximální ¹íøku, a tím zlep¹it výsledky klasifikátoru oproti pøedchozí, men¹í maximální ¹íøce.
...
Abychom byli schopni minimalizovat míru chybovosti (penalizaci za ¹patnou klasifikaci a ¹íøkove (marginové) chyby -- body le¾í uvnitø maximální ¹íøky) vzhledem k maximalizaci maximální ¹íøky, zavedeme konstantu $C > 0$, kterou budeme míru chybovosti pøenásobovat.
...
Pro potøeby implementovaného klasifikátoru rozdìlujeme pøíznaky na dvì skupiny:
\begin{itemize}
...
Následnì popí¹eme práci naivního Bayesovského klasifikátoru a klasifikátoru SVM s vytvoøenými pøíznaky.
...
Speciální pøíznaky v textu jsou mnohdy v textu skryté a klasickými pøístupy pro získávání textových pøíznakù z textu (=tokenizací) je èasto nejsme schopni z textu získat.
Tyto speciální pøíznaky jsou nositeli dùle¾ité informace proto, ¾e mohou z textu napøíklad urèovat náladu pisatele, zobecòovat informace obsa¾né v textu atd., a tím umo¾nit zpøesnìní klasifikace.
...
\begin{itemize}
  \item \emph{Celý tag} -- celý tag nalezený v textu je pou¾it jako pøíznak pro klasifikaci.
  \item \emph{Existence tagù ANO/NE} -- jestli¾e se v textu nacházejí nìjaké tagy u¾ivatelù, je vytvoøen pøíznak \emph{ANO}, jestli¾e se nenacházejí, je vytvoøen pøíznak \emph{NE}.
  \item \emph{Existence tagù ANO} -- obdobnì jako v pøípadì typu \emph{Existence tagù ANO/NE}, ale pokud ¾ádný tag nebyl nalezen, pøíznak NE není vytvoøen.
\end{itemize}
...
Pro pøevedení slov do obecnìj¹ího tvaru lze kromì stematizace pou¾ítaké takzvané lematizace, která na rozdíl od stematizace nepøevede slova na koøeny, ale do jejich základního tvaru. 
Pro tuto práci v¹ak byla zvolena metoda stematizace.

\subsubsection{Základní pøístup k získávání texových pøíznakù}
...
\subsubsection{Základní pøístup se stematizací}
...
\subsubsection{Alternativní pøístup k získávání texových pøíznakù}
Pøi vyu¾ití základního pøístupu k získávání texových pøíznakù je problémem skuteènost, ¾e vytvoøené pøíznaky jsou zcela samostatné bloky, které se pou¾ité klasifikátor (a» u¾ Bayesovský klasifikátor, nebo klasifikátor SVM), uèí ani¾ by byly schopny z tìchto textových pøíznakù urèit kontext.
V¾dy» jak bylo ji¾ bylo vý¹e zmínìno, Bayesovský klasifikátor je oznaèován jako naivní právì proto, ¾e nebere v potaz závislosti jednotlivých textových pøíznakù na sobì (co¾ vychází z odvození rovnice \ref{equation_used} pou¾ívané pro klasifikaci -- viz. \ref{SF-BAYES-MAT-bayes_theorem-deriv}).
Proto jsem se sna¾il tuto nevýhodu kompenzovat tím, ¾e jsem pou¾il alternativní pøístup k získávání texových pøíznakù zachovávající nejbli¾¹í kontext textových pøíznakù.
Ten na rozdíl od základního pøístupu k získávání textových tokenù nevytváøí pøíznaky pouze z jednotlivých slov, ale také z $N$ za sebou jdoucích slov, èím¾ bere v potaz jejich kontext ve vìtì.
...
Tato vlastnost ale opìt mù¾e být alespoò èásteènì kompenzována stematizací jednotlivých slov, a tím i sní¾ením celkového poètu rùzných textových pøíznaku.
Uka¾me si alternativní pøístup k získávání texových pøíznakù na pøíkladu:
...
Pro rozdìlení vstupního textu na jednotlivé vìty knihovna NLTK (viz. \ref{IMP-NLTK}).
...
Pøed tím, ne¾ se samozøejmì ze slov vytvoøí textové pøíznaky, pøevedou se v¹echna slova na koøeny.
Textové pøíznaky pro vstupní text a $N = 3$ budou tedy pro první vìtu vypadat následovnì:
$$['Damn', 'headache', ('Damn', 'headache')]$$
a pro druhou vìtu:
$$['I', 'should', 'be', 'sleep', ['I', 'should'], ['should', 'be'], ['be', 'sleep'], ['I', 'should', 'be'], ['should', 'be', 'sleep']]$$
...
Bayesovský klasifikátor bere postupnì v¹echny textové a speciální pøíznaky z trénovací mno¾iny a v závisosti na manuálnì pøiøazených tøídách trénovacích dat poèítá pravdìpodobnosti s jakými dané textové a speciální pøíznaky nále¾ejí do tøídy spamu.
...
\section{Pøíznaky v SVM klasifikátoru}

Na rozdíl od Bayesovského klasifikátoru, problém volby vhodných textových a speciálních pøíznaky u SVM klasifikátoru odpadá.
...
Jeliko¾ speciálních pøíznaky generují jen malé mno¾ství pøíznakù, tento problém by nemìl nastat.
...
\texttt{NLTK}, neboli Natural Language ToolKit je balík knihoven pro skriptovací jazyk Python 2.7, urèený pro symbolické a statistické zpracovávání pøirozeného jazyka.
...
V této èásti práce bude popsána architekturu programu a krátký popis tøíd.
Tato kapitolá má za cíl pøiblí¾it architekturu a funkènost programu.
...
Jejím úkolem je text rozdìlit na pøíznaky tak, aby samotné klasifikátory ji¾ s textem nemusely nijak pracovat, ale aby dostaly pouze seznam textových a speciálních pøíznakù, je¾ vstupní text obsahoval.
...
Tento slovník pøíznakù obsahuje pravdìpodobnostní hodnoty jednotlivých pøíznakù nalezených bìhem uèení a pravdìpodobnost jejich nále¾itosti do urèité tøídy. 
...
Je toti¾ vysoce pravdìpodobné, ¾e pokud bude textových pøíznakù mnohem více speciálních pøíznakù, budou mít speciální pøíznaky velmi malý dopad na výslednou klasifikaci.
...
Pøi implementaci programu jsme se rozhodli ze studijních dùvodù vytvoøit vlastní implementaci SVM klasifikátoru na místo pou¾ití ji¾ hotových implementací tohoto algoritmu.
Podobnì jako pøi implementaci Bayesovského klasifikátoru nastly i pøi implementaci klasifikátoru, zalo¾eného na support vector machines urèité problémy, z nich¾ zmíníme následující tøi:
...
V programu je mo¾né zvolit, zda chce u¾ivatel pou¾ívat promìnnou $C$ (viz \ref{SVM-THEORY-MARGINS}) --tedy zda umo¾ní poru¹ovat maximální ¹íøku \ref{SVM-THEORY-MARGINS}).
...
\chapter{Vyhodnocení systému} \label{TESTY}
...
Závìrem této kapitoly porovnáme oba implementované klasifikátory a shrneme výsledky, ke kterým jsme pøi vyhodnocování testù do¹li.

\section{Data}\label{TEST-DATA}
Jak bylo ji¾ zmínìno v úvodu (\ref{UVOD-MOTIVACE}), tato práce vznikla s cílem vytvoøit pro projekt M-Eco~\footnote{\url{http://www.meco-project.eu/}}, ve kterém participovala výzkumná skupina NLP, pracující pøi Fakultì informaèních technologii VUT v Brnì.
Hlavním cílem projektu M-Eco bylo pomocí analýzy dat ze sociálních sítí, zpravodajských serverù, blogù a dal¹ích zdrojù na internet hledat mo¾ná obecná zdravotní ohro¾ení (epidemie, pandemie, atd.).
Úkolem klasikátoru v projektu M-Eco byla klasifikace vstupního textu do tøíd a následné urèení, zda bude vstupní text zapsán do databáze a nebo nebude.
...
Datová sada obsahující anglické tweety je rozdìlena do dvou tøíd -- \textbf{zabývající se osobními zku¹enostmi pisatele s nemocemi (ale také zku¹enostmi z pisatelova okolí)}, nebo \textbf{nezabývající se tìmito zku¹enostmi}.
...
Tudí¾ napøíklad èlánek popisující vypuknutí meningitidy v USA je relevantní.
Naproti tomu èlánek popisující prùbìh a léèbu meningitidy je irelevantní.
Tyto tøídy byly zvoleny právì kvùli tomu, ¾e klasifikátory byly implementovány pro projekt M-Eco.
...
Pro testování byla pou¾ívána metoda \emph{$n$-násobná køí¾ová validace ($n$-fold cross-validation)}.
...
Jako celkový výsledek \emph{$n$-násobné køí¾ové validace} se pou¾ijí prùmìrné výsledky testù jednotlivých bìhù.
...
$$
  Accuracy = \frac{spravne\_pozitivni + spravne\_negativni}{spravne\_pozitivni + spravne\_negativni + falesne\_pozitivni + falesne\_negativni}
$$
...
De facto jde o geometrický prùmìr precision a recall.
...
V následující èásti textu boudou pøedstaveny výsledky testù Bayesovského klasifikátoru.
...
Následující tabulka øíká, jak se klasifikátor choval pøi zmìnì nastavení maximální délky za sebou jdoucích $n$-tic textových pøíznakù:
...
Je zøejmé, ¾e je-li délka maximálních $n$-tice vìt¹í ne¾ dva, Bayesovský klasifikátor klasifikuje stejnì pøesnì jako pøi pou¾ívání maximálních $n$-tic délky dva, av¹ak velikost klasifikaèního slovníku Bayesovského klasifikátoru naprosto zbyteènì roste.
...
Pou¾ijeme stejný test jako v pøípadì testu tvorby textových pøíznakù na datové sadì tweetù.
Budeme se sna¾it urèit optimální délku $n$-tic textových pøíznakù.
...
Je zøejmé, ¾e výsledky testování maximální délky $n$-tic texotvých pøíznakù nad datovou sadou anglických èlánkù dopadly prakticky stejnì jako testy nad datovou sadou anglických tweetù.
Délka $n$-tic rovna dvìma klasifikaèní schopnosti Bayesovského kalsifikátoru zlep¹uje, av¹ak del¹í vìt¹í $n$-tice ji¾ na klasifikaci nemají výrazný vliv a zbyteènì zvìt¹ují velikost slovníku Bayesovského klasifikátoru pou¾ívaného pro klasifikaci.

Dùvod pro to, ¾e umo¾nìní vyu¾ití del¹ích $n$-tic ji¾ nezlep¹uje klasifikaèní schopnosti Bayesovského klasifikátoru je zøejmì ten, ¾e tøi za sebou jdoucí slova jsou ji¾ dosti specifickým pøíznakem, který se v textu nevyskytuje pøíli¹ èasto, a tudí¾ nepøispívají klasifikátoru ke zlep¹ení klasifikaèních schopností.
...
Abychom mohli otestovat vliv jednotlivých typù speciálních pøíznakù na klasifikaci Bayesovského klasifikátoru, spustíme nejprve test klasifikátoru vyu¾ívajícího pouze textových pøíznakù.
...
Pro datovou sadu anglických tweetù byly implementovaným algoritmem pro hledání optimálních speciálních pøíznakù nalezeny následující speciální pøíznaky (viz \ref{TOKENIZACE-SPEC}):

\begin{itemize}
\item \emph{E-mailové adresy} -- jsou vytvoøeny speciální pøíznaky \emph{ANO} respektive \emph{NE}, pokud se ve vstupním textu nachází, respektive nenachází e-mailová adresa.
\item \emph{Emotikony} -- jako speciální pøíznaky jsou brány emoce vyplývající z emotikonù nalezených v textu.
\item \emph{Tagy u¾ivatelù} -- je vytvoøen speciální pøíznak \emph{ANO}, jestli¾e je ve vstupním textu nalezen tag u¾ivatele.
\end{itemize}

Ostatní skupiny speciálních pøíznakù, které zde nebyly zmínìny (URL, tagy, vìty, èasy a data) výsledky klasifikace Bayesovského klaisfikátoru nad datovou sadou tweetù nezlep¹ily, a proto nebyly pou¾ity.
...
Z tabulky vyplývá, ¾e otpimálnì zvolené speciální pøíznaky mouhou mírnì vylep¹it klasifikaèní schopnosti Bayesovského klasifikátoru pøi klasifikaci anglických tweetù.
...
Byly nalezeny následující optimální speciální pøíznaky, které nejlépe napomáhají zlep¹ení klasifikaèních schopností Bayesovského klasifikátoru pøi klasifikaci datové sady anglických èlánkù:
...
Z tabulky je zøejmé, ¾e pro datovou sadu èlánkù speciální pøíznaky sice o nìco zlep¹ily korelaci klasifikátoru, nicménì na výsledncích klasifikace Bayesovského klasifikátoru se to nikterak neprojevilo.
Takto malý vliv na zlep¹ení klasifikace jak pøi klasifikaci anglických èlánkù, zpùsoben zøejmì tím, ¾e se ve èláncích na rozdíl od tweetù vyskytuje daleko ménì implementovaných speciálních pøíznakù, a to pøedev¹ím proto, ¾e èlánky bývají na rozdíl od tweetù psány formálnìji, a proto se tam napøíklad nevyskytují emotikony a podobnì.
...
Klasifikátoru umo¾níme si nalézt optimální øe¹ení ze v¹ech pøíznakù (textových i speciálních), které implementovaný program umo¾òuje vytvoøit.
Na druhou stranu stejnì jako v Bayesovském klasifikátoru budeme hledat optimální délku $n$-tic textových pøíznakù, která by mìla být taková, aby zlep¹ovala klasifikaèní schopnosti klasifikátoru.
...
Ve¹keré testy s SVM klasifikátorem byly provádìny za pou¾ití RBF jaderné funkce a optimální parametry SVM a jaderné funkce byly nalezeny pomocí metody simulovaného ¾íhání.
...
Z tabulky vý¹e jasnì vyplývá, ¾e klasifikaèní schopnosti SVM klasifikátoru se se zvy¹ováním maximální délky $n$-tic textových pøíznakù zhor¹ovaly, tudí¾ není vhodné tyto $n$-tice pro klasifikaci tweetù pomocí SVM klasifikátoru pou¾ívat. 
...
\subsection{Èlánky}
Stejnì jako pro datovou sadu anglických tweetù jsme na datové sadì anglických èlánkù provedli testy optimální délky $n$-tic textových pøíznakù.
...
Pro maximální délku $n$-tic rovnou tøem ji¾ test klasifikátoru nedobìhl, jeliko¾ test byl pøíli¹ nároèný na RAM pamì» poèítaèe, na kterém testy bì¾ely.
...
Z tabulky vý¹e je, stejnì jako pøi klasifikace tweetù, jasné, ¾e nejlep¹ích klasifikaèních výsledkù dosahuje SVM klasifikátor kdy¾ jsou pou¾ívány pouze základní jednoslovní textové pøíznaky.
...
Pro oba klasifikátory vybereme optimální nastavení pøíznakù, které jsme získali pomocí testù vý¹e a porovnáme vysledky klasifikátorù na obou vytvoøenýchtestových sadách, tedy jak na datové sadì anglických tweetù, tak na datové sadì anglických èlánkù.
