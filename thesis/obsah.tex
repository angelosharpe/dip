...
\section{Výbìr volných parametrù klasifikátoru a jaderných funkcí}
...
Mìjme opìt text \textit{'Damned headache. I should be sleeping.'}.
...
Podobnì jako pøi implementaci Bayesovského klasifikátoru nastly i pøi implementaci klasifikátoru, zalo¾eného na support vector machines urèité problémy, z nich¾ zmíníme následující dva:
...
Následující tabulka popisuje jak moc jednotlivé pøíznaky zlep¹ovaly, nebo zhor¹ovaly výsledek klasifikace oproti kontrolnímu výsledku.
Porovnáváme hodnoty korelace (kladná hodnota znamená zlep¹ení a záporná zhor¹ení klasifikaèních schopností testovaného Bayesovského klasifikátoru).
...
Z tabulky vyplývá, ¾e optimální speciální tokeny pro datovou sadu anglických tweetù jsou následující:
...
Ostatní skupiny speciálních pøíznaky (URL, e-mailové adresy, tagy, vìty, èasy a data) výsledky klasifikace Bayesovského klaisfikátoru nad datovou sadou tweetù nezlep¹ily, a proto nebyly pou¾ity.
...
Z tabulky vyplývá, ¾e otpimálnì zvolené speciální pøíznaky mohou mírnì vylep¹it klasifikaèní schopnosti Bayesovského klasifikátoru pøi klasifikaci anglických tweetù.
...
Následující tabulka, stejnì jako v pøípadì anglických tweetù, popisuje jak moc jednotlivé pøíznaky zlep¹ovaly, nebo zhor¹ovaly výsledek klasifikace oproti kontrolnímu výsledku.
Opìt porovnáváme hodnoty korelace (kladná hodnota znamená zlep¹ení a záporná zhor¹ení klasifikaèních schopností testovaného Bayesovského klasifikátoru).
...
Stejnì jako pøi testech provádìných na datové sadì tweetù Bayesovsky klasifikátor pou¾íval $n$-tice textových pøíznakù o maximální délce rovné dvìma a vhodnì zvolené speciální pøíznaky.
...
Stejnì jako v pøípadì klasifikace datové sady tweetù jsou pøi klasifikaci anglických èlánkù klasifikaèní schopnosti SVM klaisfikátoru lep¹í, ne¾ klasifikaèní schopnosti Bayesovského klasifikátoru.
V pøíladì anglických èlánkù je ale rozdíl mezi výsledky klasifikace Bayesovského klasifikátoru a SVM klasifikátoru propastný -- Bayesovský klasifikátor dosáhl ohodnocení pomocí metriky F1-measure rovné 0.6485 a SVM klasifikátor dosáhl ohodnocení 0.8232, co¾ je velmi velký rozdíl ve prospìch SVM klasifikátoru.

\section{Shrnutí výsledkù testù}
Pro Bayesovský klasifikátor bylo na¹ím cílem v pøípadì textových pøíznakù experimentálnì nalézt optimální délku $n$-tic textových pøíznakù a v pøípadì speciálních pøíznakù zvolit takové speciální pøíznaky, které co nejvíce napomáhají zlep¹ení klasifikaèních schopnstí implementovaného Bayesovského klasifikátoru pøi klasifikaci datových sad vytvoøených pro testování.
U obou datových sad jsme zjistili, ¾e optimální délka $n$-tic textových pøíznakù je rovna dvìma a ¾e textové pøíznaky mají pøí vhodné volbì délky $n$-tic textových pøíznakù pomìrnì velký vliv na zlep¹ení klasifikaèních schopností Bayesovského klaisfikátoru.
...
Pøi klasifikaci anglických èlánkù nemìly na klasifikaci prakticky ¾ádný vliv.
...
\chapter{Závìr} 
V této diplomové práci jsme se zamìøili na porovnání dvou klasifikaèních metod pro klasifikaci textu -- SVM klasifikátoru a Bayesovského klasifikátoru.
Obì tyto metody jsme podrobnì analyzovali a popsali jejich matematický model.

V celé diplomové práci jsme kladli velký dùraz  na praktickou vyu¾itelnost implementovaných klasifikátorù.

Abychom co nejvìt¹í mìrou pozitivné ovlivnili klasifikaèní schopnosti obou implementovaných klasifikátorù, tak jsme dbali na výbìr vhodných pøíznakù extrahovaných z klasifikovaného textu.
...
Do speciálnich pøíznakù potom patøily ostatní netextové pøíznaky, které byly z textu získány -- napø. emotikony, poèet slov ve vìtì, e-mailové adresy atd. (viz \ref{TOKENIZACE}).
...
Mimo výbìru optimálních pøíznakù bylo tøeba pro SVM klasifikátor urèit optimální nastavení volných parametrù klasifikátoru a pou¾ité jaderné funkce (viz \ref{SVM}, \ref{SVM-KERNELS}).
Pro jejich zji¹tìní jsme pou¾ili algoritmu simulovaného ¾íhání.

Pro testování jsme anotovali dvì testovací datové sady (datová sada obsahující tweet -- zprávy ze sociální sítì twitter a datová sada anglických èlánkù), na kterých jsme následnì oba implementované klasifikátory testovali a pro ne¾ jsme se sna¾ili zvolit jaké optimální textové a speciální pøiznaky pou¾ívat, abychom co nejvíce pozitivnì ovlivnili klasifikaèní schopnosti implementovaného SVM a Bayesovského klasifikátoru.
...
Na závìr jsme vyu¾ili znalosti optimálních nastavení obou klasifikátorù pro klasifikaci obou námi vytvoøených testovacích datových sad a porovnali jsme jejich klasifikaèní schopností.
Podle výsledkù tohoto porovnání je SVM klasifikátor jasnì lep¹í ne¾ Bayesovský klasifikátor (viz \ref{porovnani}).

V pøípadì pokraèování na této práci by bylo vhodné vytvoøit vìt¹í sadu speciálních pøíznakù a otestovat jejich vliv na klasifikaèní schopnosti klasifikátorù.
S velmi dobrými výsledky by zøejmì mohla být pro oba implementované klasifkikátory pou¾ita metoda zvaná boosting popsaná v èlánku Michael Kearnse s názvem \uv{Thoughts on hypothesis boosting} \cite{Kearns1988} (viz také \ref{boosting}).
...
SVM klasifikátor implementovaný v této diplomové práci v¹ak ji¾ bohu¾el pou¾it nebyl, a to z dùvodu ukonèení spolupráce na projektu M-Eco v prosinci roku 2012.
