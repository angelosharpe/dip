...
Pro tuto práci byly zvoleny dvì klasifikaèní metody, jedna probabilistická a jedna neprobabilistická.
...
Nalezení specifické diskriminaèní funkce s maximálním geometrickým marginem je tedy ekvivalentní s následujícím optimalizaèním problémem:
  ...
  minimalizace		 $\frac{1}{2}||w||^2$
\begin{equation}
  za podmínek: y_n(w^T x_i + b) \geq 1, kde n = 1,\ldots,l
\end{equation}
...
Takto upravený problém nyní vypadá následovnì:
  ...
  minimalizace		 $\frac{1}{2}||w||^2$
\begin{equation}
  za podmínek: y_n(w^T x_i + b) \geq 1 - \xi_i, kde n = 1,\ldots,l
\end{equation}
...
Na¹e optimalizaèní úloha tedy s touto soft-margin konstantou vypadá následovnì:
  ...
  minimalizace		 $\frac{1}{2}||w||^2 + C \sum^{n}_{i=1}\xi_i$
\begin{equation}
  za podmínek: y_n(w^T x_i + b) \geq 1 - \xi_i, kde n = 1,\ldots,l
\end{equation}
...
za podmínek:
\begin{equation}
  \sum^n_{y_i\alpha_i = 0}, 0 \leq \alpha_i \leq C
\end{equation}
...
Celý program je implementován v programovacím jazyku Python.
...
\section{Architektura programu a implementace obecných èástí}
V této èásti kapitoly o implementaci si ve zkratce pøedstavíme architekturu implementovaného programu a implementaci obecných èástí programu.


\subsection{Architektura programu}
V koøenovém adresáøi tohoto projektu se nachází spou¹tìcí skript, pomocí kterého je program ovládaný a který nabízí ve¹keré operace, které se dají s programem provést.
...
\subsection{Implementace obecných èástí programu}
...
\section{Implementace klasifikátoru SVM}
...
%False positive = 8.4
%False negative = 8.4
%Precision = 0.917485265226
%Recall = 0.917485265226
%Accuracy = 0.91674925669
%F-measure = 0.917485265226
%##################################
%
%##################################
%########## bayesrestults ##########
...
%Precision = 0.902
%Recall = 0.902
%Accuracy = 0.902195608782
%F-measure = 0.902
%Corelation = 0.826996274323
%##################################
%{'emoticon': 1, 'sentence': 1, 'url': 4, 'tag': 1, 'time': 0, 'date': 0, 'email': 0}$$
